...
namespace LavaLeak.Diplomata.Editor.Windows
{
    public class ItemListMenu : EditorWindow
    {
        ...
        {
            var window = (ItemListMenu) GetWindow(typeof(ItemListMenu), false, "Inventory");
        ...
        public void OnGUI()
        {
            ...
            foreach (var item in Controller.Instance.Inventory.items)
            {
                if (item.SetId())
                {
                    InventoryController.Save(Controller.Instance.Inventory,
                ...
                if (GUILayout.Button("Delete", GUILayout.Height(GUIHelper.BUTTON_HEIGHT_SMALL)))
                {
                        ...
                        "Do you really want to delete?\nThis data will be lost forever.", "Yes", "No"))
                    {
                        ...
                        Controller.Instance.Inventory.items =
                            ArrayHelper.Remove(Controller.Instance.Inventory.items, item);
...
//        }
            ...
            {
                Controller.Instance.Inventory.items = ArrayHelper.Add(Controller.Instance.Inventory.items,
                    new Item(Controller.Instance.Inventory.GenerateId(), Controller.Instance.Options));
                InventoryController.Save(Controller.Instance.Inventory, Controller.Instance.Options.jsonPrettyPrint);
            }
