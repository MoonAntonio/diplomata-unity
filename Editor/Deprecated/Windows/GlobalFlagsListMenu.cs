...
namespace LavaLeak.Diplomata.Editor.Windows
{
    public class GlobalFlagsListMenu : EditorWindow
    {
        ...
        {
            var window = (GlobalFlagsListMenu) GetWindow(typeof(GlobalFlagsListMenu), false, "Custom Flags");
        ...
        public void OnGUI()
        {
            ...
            for (var i = 0; i < Controller.Instance.GlobalFlags.flags.Length; i++)
            {
                ...
                flag.name = EditorGUILayout.TextArea(flag.name, GUIHelper.textAreaStyle, GUILayout.ExpandWidth(true),
                ...
                var selected = flag.value.ToString();
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    GlobalFlagsController.Save(Controller.Instance.GlobalFlags,
                ...
                if (GUILayout.Button("Up", GUILayout.Height(GUIHelper.BUTTON_HEIGHT_SMALL)))
                {
                    if (i > 0)
                    {
                        Controller.Instance.GlobalFlags.flags =
                            ArrayHelper.Swap(Controller.Instance.GlobalFlags.flags, i, i - 1);
                        GlobalFlagsController.Save(Controller.Instance.GlobalFlags,
                            Controller.Instance.Options.jsonPrettyPrint);
                    }
                }
                ...
                {
                    Controller.Instance.GlobalFlags.flags =
                        ArrayHelper.Add(Controller.Instance.GlobalFlags.flags, new Flag("", false));

                    for (var j = 1; j < Controller.Instance.GlobalFlags.flags.Length - 1 - i; j++)
                    {
                        Controller.Instance.GlobalFlags.flags = ArrayHelper.Swap(Controller.Instance.GlobalFlags.flags,
                            Controller.Instance.GlobalFlags.flags.Length - 1, i + j);
                    }
                ...
                if (GUILayout.Button("Delete", GUILayout.Height(GUIHelper.BUTTON_HEIGHT_SMALL)))
                {
                    ...
                    {
                        Controller.Instance.GlobalFlags.flags =
                            ArrayHelper.Remove(Controller.Instance.GlobalFlags.flags, flag);
                        GlobalFlagsController.Save(Controller.Instance.GlobalFlags,
                            Controller.Instance.Options.jsonPrettyPrint);
                    }
                }
