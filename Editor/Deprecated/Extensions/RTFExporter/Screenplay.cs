...
namespace LavaLeak.Diplomata.Editor.Extensions
{
    public class Screenplay
    {
        ...
        public static void SaveAll()
        {
            ...
            RTFParser.ToFile(
                "Assets/" + PlayerSettings.productName + " Screenplay - " + options.currentLanguage + ".rtf", doc);
        ...
        public static void Save(Talkable talkable)
        {
            ...
            RTFParser.ToFile(
                "Assets/" + PlayerSettings.productName + " Screenplay - " + talkable.name + " - " +
                options.currentLanguage + ".rtf", doc);
            AssetDatabase.Refresh();
        }

        private static RTFDocument CreateDocument()
        {
            var document = new RTFDocument(21, 29.7f, Orientation.Portrait, Units.Centimeters);
        ...
        {
            var style = new RTFTextStyle(false, false, 12, "Courier", Color.black);
            var styleAllcaps = new RTFTextStyle(false, false, false, false, true, false, 12, "Courier", Color.black,
                Underline.None);

            var noIndent = new RTFParagraphStyle(Alignment.Left, new Indent(0, 0, 0), 0, 400);
            var messageContentIndent = new RTFParagraphStyle(Alignment.Left, new Indent(0, 2.54f, 0), 0, 400);
            ...
            for (var i = 0; i < character.contexts.Length; i++)
            {
                ...
                foreach (var column in context.columns)
                {
                    for (var j = 0; j < column.messages.Length; j++)
                    {
                        ...
                        var screenplayNotes = DictionariesHelper.ContainsKey(column.messages[j].screenplayNotes,
                        ...
                        var content =
                            DictionariesHelper.ContainsKey(column.messages[j].content, options.currentLanguage);
