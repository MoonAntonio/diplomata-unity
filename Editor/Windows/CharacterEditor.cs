...
namespace LavaLeak.Diplomata.Editor.Windows
{
    public class CharacterEditor : EditorWindow
    {
        ...
        public static void Init(State state = State.None)
        {
            ...
            var window = (CharacterEditor) GetWindow(typeof(CharacterEditor), false, "Character", true);
        ...
        public void Create()
        {
            if (characterName != "")
            {
                CharactersController.AddCharacter(characterName, Controller.Instance.Options,
                    Controller.Instance.Characters);
            }
            else
            {
        ...
        public void DrawEditWindow()
        {
            ...
            var description =
                DictionariesHelper.ContainsKey(character.description, Controller.Instance.Options.currentLanguage);

            if (description == null)
            {
                character.description = ArrayHelper.Add(character.description,
                    new LanguageDictionary(Controller.Instance.Options.currentLanguage, ""));
                description = DictionariesHelper.ContainsKey(character.description,
                    Controller.Instance.Options.currentLanguage);
            }

            GUIHelper.textContent.text = description.value;
            var height =
                GUIHelper.textAreaStyle.CalcHeight(GUIHelper.textContent, position.width - 2 * GUIHelper.MARGIN);

            GUILayout.Label("Description: ");
            description.value =
                EditorGUILayout.TextArea(description.value, GUIHelper.textAreaStyle, GUILayout.Height(height));
            ...
            if (character.name != Controller.Instance.Options.playerCharacterName)
            {
                ...
                foreach (var attrName in Controller.Instance.Options.attributes)
                {
                    ...
                    else
                    {
                        for (var i = 0; i < character.attributes.Length; i++)
                        {
                            ...
                            {
                                character.attributes = ArrayHelper.Add(character.attributes,
                ...
                for (var i = 0; i < character.attributes.Length; i++)
                {
                    ...
                    {
                        character.attributes[i].value = (byte) EditorGUILayout.Slider(character.attributes[i].key,
                            character.attributes[i].value, 0, 100);
                    }
                }
