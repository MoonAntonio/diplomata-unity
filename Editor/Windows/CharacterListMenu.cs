...
namespace LavaLeak.Diplomata.Editor.Windows
{
    public class CharacterListMenu : EditorWindow
    {
        ...
        {
            var window = (CharacterListMenu) GetWindow(typeof(CharacterListMenu), false, "Character List");
        ...
        public void OnGUI()
        {
            ...
            for (var i = 0; i < Controller.Instance.Options.characterList.Length; i++)
            {
                ...
                if (GUILayout.Button("Delete", GUILayout.Height(GUIHelper.BUTTON_HEIGHT_SMALL)))
                {
                        ...
                        "Do you really want to delete?\nThis data will be lost forever.", "Yes", "No"))
                    {
                        ...
                        Controller.Instance.Options.characterList =
                            ArrayHelper.Remove(Controller.Instance.Options.characterList, name);

                        JSONHelper.Delete(name, "Diplomata/Characters/");

                        if (isPlayer && Controller.Instance.Options.characterList.Length > 0)
                        {
                            Controller.Instance.Options.playerCharacterName =
                                Controller.Instance.Options.characterList[0];
                        }

                        OptionsController.Save(Controller.Instance.Options,
                            Controller.Instance.Options.jsonPrettyPrint);
                        Controller.Instance.Characters =
                            CharactersController.GetCharacters(Controller.Instance.Options);
