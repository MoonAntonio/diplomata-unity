...
namespace LavaLeak.Diplomata.Editor.Windows
{
    public class QuestEditor : EditorWindow
    {
        ...
        public static void Init(State state = State.None)
        {
            ...
            var window = (QuestEditor) GetWindow(typeof(QuestEditor), false, "Quest", true);
        ...
        public void OnGUI()
        {
            ...
            switch (state)
            {
                ...
                case State.CreateEdit:
                    ...
                    if (questName == null)
                    {
                        quest.Name = ArrayHelper.Add(quest.Name,
                    ...
                    DictionariesHelper.ContainsKey(quest.Name, Controller.Instance.Options.currentLanguage).value =
                        EditorGUILayout.TextField(DictionariesHelper
                            .ContainsKey(quest.Name, Controller.Instance.Options.currentLanguage).value);
                    ...
                    foreach (var questState in quest.questStates)
                    {
                        ...
                        var questStateShortDescription = DictionariesHelper.ContainsKey(questState.ShortDescription,
                            Controller.Instance.Options.currentLanguage);
                        if (questStateShortDescription == null)
                        {
                            questState.ShortDescription = ArrayHelper.Add(questState.ShortDescription,
                                new LanguageDictionary(Controller.Instance.Options.currentLanguage, ""));
                        }

                        DictionariesHelper.ContainsKey(questState.ShortDescription,
                                Controller.Instance.Options.currentLanguage).value =
                            EditorGUILayout.TextArea(DictionariesHelper.ContainsKey(questState.ShortDescription,
                                Controller.Instance.Options.currentLanguage).value);
                        EditorGUILayout.Space();

                        EditorGUILayout.LabelField("Long description:");

                        var questStateLongDescription = DictionariesHelper.ContainsKey(questState.LongDescription,
                            Controller.Instance.Options.currentLanguage);
                        if (questStateLongDescription == null)
                        {
                            questState.LongDescription = ArrayHelper.Add(questState.LongDescription,
                                new LanguageDictionary(Controller.Instance.Options.currentLanguage, ""));
                        }

                        DictionariesHelper.ContainsKey(questState.LongDescription,
                                Controller.Instance.Options.currentLanguage).value =
                            EditorGUILayout.TextArea(DictionariesHelper.ContainsKey(questState.LongDescription,
                                Controller.Instance.Options.currentLanguage).value);
                        EditorGUILayout.Space();

                        EditorGUILayout.SelectableLabel(string.Format("Quest State Unique ID: {0}",
                            questState.GetId()));
                        ...
                        if (GUILayout.Button("Up", GUILayout.Height(GUIHelper.BUTTON_HEIGHT_SMALL)))
                        {
                            if (index > 0)
                            {
                                ...
                                QuestsController.Save(Controller.Instance.Quests,
                                    Controller.Instance.Options.jsonPrettyPrint);
                            }
                        }
