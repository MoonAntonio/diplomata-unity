...
namespace LavaLeak.Diplomata
{
    ...
    public class DiplomataTalkable : MonoBehaviour
    {
                                        ...
                                        foreach (var condition in msg.conditions)
                                        {
                                            switch (condition.type)
                                            {
                                                ...
                                                case Condition.Type.HasItem:
                                                    var item = Item.Find(DiplomataManager.Data.inventory.items,
                                                        condition.itemId);
                                                    ...
                                                    {
                                                        Debug.LogWarning(
                                                            "Cannot find the item with id " + condition.itemId +
                                                            " to check.");
                                                        condition.proceed = false;
                                                    }
                                                ...
                                                case Condition.Type.DoesNotHaveTheItem:
                                                    var itemDont = Item.Find(DiplomataManager.Data.inventory.items,
                                                        condition.itemId);
                                                ...
                                                case Condition.Type.ItemIsEquipped:
                                                    var equippedItem = Item.Find(DiplomataManager.Data.inventory.items,
                                                        condition.itemId);
                                                ...
                                                case Condition.Type.ItemWasDiscarded:
                                                    var discardedItem = Item.Find(DiplomataManager.Data.inventory.items,
                                                        condition.itemId);
                                                ...
                                                case Condition.Type.GlobalFlagEqualTo:
                                                    var flag = DiplomataManager.Data.globalFlags.Find(
                                                        DiplomataManager.Data.globalFlags.flags,
                                                    ...
                                                    {
                                                        Debug.LogWarning(
                                                            "Cannot find the custom flag " + condition.globalFlag.name);
                                                        condition.proceed = false;
                                                    }

                                                    break;
                                                case Condition.Type.QuestStateIs:
                                                    var quest = Quest.Find(DiplomataManager.Data.quests,
                                                    ...
                                                    if (quest != null)
                                                    {
                                                        ...
                                                        if (currentState == null)
                                                        {
                                                            Debug.Log(string.Format(
                                                                "Probably quest {0} is not initialized.",
                                                                quest.GetName()));
                                                            condition.proceed = false;
                                                        }
                                                    ...
                                                    {
                                                        Debug.LogWarning(
                                                            "Cannot find the quest " + condition.questAndState.questId);
                                                        condition.proceed = false;
                                                    }
                                                ...
                                                case Condition.Type.QuestInitialized:
                                                    var questInitialized = Quest.Find(DiplomataManager.Data.quests,
                                                        condition.questAndState.questId);
                                                ...
                                                case Condition.Type.QuestFinished:
                                                    var questFinished = Quest.Find(DiplomataManager.Data.quests,
                                                        condition.questAndState.questId);
                    ...
                    if (currentMessage.audioClip != null)
                    {
                        ...
                        if (audioSource != null)
                        {
                            audioSource.PlayOneShot(currentMessage.audioClip,
                                DiplomataManager.Data.options.volumeScale);
                        }

                        else
                        {
                            Debug.LogWarning(
                                "You have a audio clip in this message, but the game object don't have a AudioSource.");
                        }
                    }
                }
            }
            ...
            if (currentMessage != null)
            {
                if (currentMessage.animatorAttributesSetters.Length > 0)
                {
                    ...
                    foreach (Animator animator in animators)
                    {
                        ...
                        {
                            foreach (var animatorAttribute in currentMessage.animatorAttributesSetters)
                            {
                ...
                foreach (var effect in currentMessage.effects)
                {
                    switch (effect.type)
                    {
                        case Effect.Type.EndOfContext:
                            ...
                            {
                                effect.endOfContext.GetContext(DiplomataManager.Data.characters,
                                    DiplomataManager.Data.interactables).Finished = true;
                        ...
                        case Effect.Type.SetAnimatorAttribute:
                            ...
                            foreach (Animator animator in animators)
                            {
                                if (animator.runtimeAnimatorController != null)
                                {
                                    ...
                                    if (effect.animatorAttributeSetter.animator != null)
                                    {
                                        if (animator.runtimeAnimatorController.Equals(effect.animatorAttributeSetter
                                            .animator))
                                        {
                                            switch (effect.animatorAttributeSetter.type)
                                            {
                                                case AnimatorControllerParameterType.Bool:
                                                    animator.SetBool(effect.animatorAttributeSetter.name,
                                                        effect.animatorAttributeSetter.setBool);
                                                    break;
        ...
        public string PlayerName() => DiplomataManager.Data.options.playerCharacterName;
        ...
        {
            foreach (var col in currentContext.columns)
            {
        ...
        public string GetLastMessageContent()
        {
            ...
            return DictionariesHelper
                .ContainsKey(lastMessage.attachedContent[lastMessage.attachedContent.Length - 1].content,
                    DiplomataManager.Data.options.currentLanguage).value;
        }
