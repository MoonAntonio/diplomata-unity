...
namespace LavaLeak.Diplomata.Models.Collections
{
    ...
    public class GlobalFlags : Data
    {
        ...
        public Flag Find(string name) => (Flag) Helpers.Find.In(flags).Where("name", name).Result;
        ...
        /// <param name="name">The name of the flag.</param>
        /// <returns>The flag if found, or null.</returns>
        public Flag Find(Flag[] array, string name) => (Flag) Helpers.Find.In(array).Where("name", name).Result;
