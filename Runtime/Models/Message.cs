...
namespace LavaLeak.Diplomata.Models
{
    ...
    public class Message : Data
    {
        ...
        public Message(int id, string emitter, int columnId, string labelId, Options options)
        {
            ...
            foreach (var str in options.attributes)
            {
        ...
        private string SetUniqueId()
        {
            ...
            var data = md5Hash.ComputeHash(Encoding.UTF8.GetBytes(
                "Diplomata" + id + columnId + UnityEngine.Random.Range(-2147483648, 2147483647)));
                ...
                foreach (var column in context.columns)
                {
                    foreach (var message in column.messages)
                    {
                        foreach (var content in message.content)
                        {
                            ...
                            {
                                contents = ArrayHelper.Add(contents, content.value);
                            }
                        }
                    }
                }
        ...
        public string GetContent(string language)
        {
            ...
            {
                if (langContent.key == language)
                {
                    return langContent.value;
                }
            }
        ...
        public static Message Find(Message[] array, string uniqueId) =>
            (Message) Helpers.Find.In(array).Where("uniqueId", uniqueId).Result;
        ...
        public static Message Find(Message[] array, int rowId) =>
            (Message) Helpers.Find.In(array).Where("id", rowId).Result;

        public static Message[] ResetIDs(Message[] array)
        {
            var temp = new Message[0];

            for (var i = 0; i < array.Length + 1; i++)
            {
                var msg = Find(array, i);
