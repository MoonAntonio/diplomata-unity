...
namespace LavaLeak.Diplomata
{
    ...
    public static class DiplomataManager
    {
        ...
        /// </summary>
        public static DiplomataData PreLoadData() => Data;
        ...
        public static Flag[] GetFlags() => Data.globalFlags.flags;
        ...
        public static Flag GetFlag(string name) => Data.globalFlags.Find(Data.globalFlags.flags, name);

        /// <summary>
        /// Get all data from the characters.
        /// </summary>
        /// <returns>A array of <seealso cref="Diplomata.Models.Character" />.</returns>
        public static Character[] GetCharacters() => Data.characters.ToArray();

        /// <summary>
        /// Get all data from the interactables.
        /// </summary>
        /// <returns>A array of <seealso cref="Diplomata.Models.Interactable">.</returns>
        public static Interactable[] GetInteractables() => Data.interactables.ToArray();
        ...
        public static Character GetCharacter(string name) => Character.Find(Data.characters, name);
        ...
        public static Interactable GetInteractable(string name) => Interactable.Find(Data.interactables, name);
        ...
        public static Context GetContext(Talkable talkable, int contextIndex) => Context.Find(talkable, contextIndex);
        ...
        public static Context GetContext(Talkable talkable, string contextName, string language) =>
            Context.Find(talkable, contextName, language);
        ...
        public static Quest[] GetQuests() => Data.quests;
        ...
        public static Quest GetQuest(string name) => Quest.Find(Data.quests, name);

        /// <summary>
        /// Get all data from the items in the inventory.
        /// </summary>
        /// <returns>A array of <seealso cref="Diplomata.Models.Item">.</returns>
        public static Item[] GetItems() => Data.inventory.items;
        ...
        public static Item GetItem(string name, string language = "")
        {
            if (language == "")
            {
                language = Data.options.languages[0].name;
            }
        ...
        public static Item GetItem(int itemId) => Item.Find(Data.inventory.items, itemId);
        ...
        public static CharacterPersistentContainer GetPersistentCharacters() =>
            new CharacterPersistentContainer(
                Persistence.Data.GetArrayData<CharacterPersistent>(DiplomataManager.Data.characters.ToArray()));
        ...
        public static InteractablePersistentContainer GetPersistentInteractables() =>
            new InteractablePersistentContainer(
                Persistence.Data.GetArrayData<InteractablePersistent>(DiplomataManager.Data.interactables.ToArray()));
        ...
        public static QuestPersistentContainer GetPersistentQuests() =>
            new QuestPersistentContainer(Persistence.Data.GetArrayData<QuestPersistent>(DiplomataManager.Data.quests));
        ...
        public static TalkLogPersistentContainer GetPersistentTalkLogs() =>
            new TalkLogPersistentContainer(
                Persistence.Data.GetArrayData<TalkLogPersistent>(DiplomataManager.Data.talkLogs));
        ...
        public static object[] Find(Model model, string field, object value)
        {
            switch (model)
            {
                ...
                case Model.CONTEXT:
                    ...
                    foreach (var character in Data.characters)
                    {
                        contexts = ArrayHelper.Merge(contexts,
                            (Context[]) Helpers.Find.In(character.contexts).Where(field, value).Results);
                    }

                    foreach (var interactable in Data.interactables)
                    {
                        contexts = ArrayHelper.Merge(contexts,
                            (Context[]) Helpers.Find.In(interactable.contexts).Where(field, value).Results);
                    }
                    ...
                    foreach (var character in Data.characters)
                    {
                        foreach (var context in character.contexts)
                        {
                            columns = ArrayHelper.Merge(columns,
                                (Column[]) Helpers.Find.In(context.columns).Where(field, value).Results);
                        }
                    }
                ...
                case Model.MESSAGE:
                    ...
                    foreach (var character in Data.characters)
                    {
                        foreach (var context in character.contexts)
                        {
                            foreach (var column in context.columns)
                            {
                                messages = ArrayHelper.Merge(messages,
                                    (Message[]) Helpers.Find.In(column.messages).Where(field, value).Results);
                            }
                        }
                    }
