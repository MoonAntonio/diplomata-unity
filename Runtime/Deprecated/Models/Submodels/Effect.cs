...
namespace LavaLeak.Diplomata.Models.Submodels
{
    ...
    public class Effect
    {
        ...
        public Effect(string talkableName) => endOfContext.talkableName = talkableName;

        public string DisplayNone() => "None";

        public string DisplayEndOfContext(string contextName) =>
            string.Format("End of the context \"{0}\"", contextName);

        public string DisplayGoTo(string messageTitle) => string.Format("Go to \"{0}\"", messageTitle);
            ...
            if (animatorAttributeSetter != null)
            {
                switch (animatorAttributeSetter.type)
                {
                    case AnimatorControllerParameterType.Bool:
                        return string.Format("Set animator attribute {0} to {1}", animatorAttributeSetter.name,
                            animatorAttributeSetter.setBool);

                    case AnimatorControllerParameterType.Float:
                        return string.Format("Set animator attribute {0} to {1}", animatorAttributeSetter.name,
                            animatorAttributeSetter.setFloat);

                    case AnimatorControllerParameterType.Int:
                        return string.Format("Set animator attribute {0} to {1}", animatorAttributeSetter.name,
                            animatorAttributeSetter.setInt);
        ...
        public string DisplayGetItem(string itemName) => string.Format("Get the item: \"{0}\"", itemName);

        public string DisplayDiscardItem(string itemName) => string.Format("Discard the item: \"{0}\"", itemName);

        public string DisplayEquipItem(string itemName) => string.Format("Equip the item: \"{0}\"", itemName);

        public string DisplayGlobalFlagEqualTo() =>
            string.Format("\"{0}\" set to {1}", globalFlag.name, globalFlag.value);

        public string DisplayEndOfDialogue() => "End of dialogue.";
